...
radius=3;
          if sum(objects)~=0
                ...
                for i001=1:size(objects,1)
                    ...
                    [firingratemouob,countmouob1,countTimemouob1] = calculatingCellSpatialForSingleData_adapted_mouob_not_look(neuronIndividuals_new{i},behavpos,behavtime,maxbehavROI,binsize,1:size(neuronIndividuals_new{i}.trace,1),thresh,temp,if_mouse_head_toward_object(:,i001),[],[],[],countTimeThresh);  %%%bin size suggests to be 15
                    [firingratemouob_amp,countmouob1_amp,countTimemouob1_amp] = calculatingCellSpatialForSingleData_adapted_mouob_amp_not_look(neuronIndividuals_new{i},behavpos,behavtime,maxbehavROI,binsize,1:size(neuronIndividuals_new{i}.trace,1),thresh,temp,if_mouse_head_toward_object(:,i001),[],[],[],countTimeThresh);
                    for iiiii=1:length(countmouob1)
                        mouselookobjectfiringexcel{1,1+20}='not look at obj';
                        mouselookobjectfiringexcel{iiiii+1,1+20}=iiiii;
                        mouselookobjectfiringexcel{iiiii+1,5*(i-1+kk)+3-1+20}=nameparts{i};
                    ...
                    for iiiii=1:length(countmouob1)
                        ...
                        if isequal(objname{i001,1},nameset{1})
                             mouselookobjectfiringexcel{iiiii+1,5*(i-1+kk)+3+1-1+20}=sum(sum(ct));
                        end
                        if isequal(objname{i001,1},nameset{2})
                             mouselookobjectfiringexcel{iiiii+1,5*(i-1+kk)+3+2-1+20}=sum(sum(ct));
                        end
                        if isequal(objname{i001,1},nameset{3})
                             mouselookobjectfiringexcel{iiiii+1,5*(i-1+kk)+3+3-1+20}=sum(sum(ct));
                        end
                        if isequal(objname{i001,1},nameset{4})
                             mouselookobjectfiringexcel{iiiii+1,5*(i-1+kk)+3+4-1+20}=sum(sum(ct));
                        end                      

                        mouselookobjectfiringrateexcel{iiiii+1,1+20}=iiiii;
                        mouselookobjectfiringrateexcel{iiiii+1,5*(i-1+kk)+3-1+20}=nameparts{i};
                        ...
                        ct2=ct2.*range1;
                        if isequal(objname{i001,1},nameset{1})
                            mouselookobjectfiringrateexcel{iiiii+1,5*(i-1+kk)+3+1-1+20}=sum(sum(ct2));%we already indicate the obj nearby points in behav files so no need to add them again
                        end
                        if isequal(objname{i001,1},nameset{2})
                            mouselookobjectfiringrateexcel{iiiii+1,5*(i-1+kk)+3+2-1+20}=sum(sum(ct2));%we already indicate the obj nearby points in behav files so no need to add them again
                        end
                        if isequal(objname{i001,1},nameset{3})
                            mouselookobjectfiringrateexcel{iiiii+1,5*(i-1+kk)+3+3-1+20}=sum(sum(ct2));%we already indicate the obj nearby points in behav files so no need to add them again
                        end
                        if isequal(objname{i001,1},nameset{4})
                            mouselookobjectfiringrateexcel{iiiii+1,5*(i-1+kk)+3+4-1+20}=sum(sum(ct2));%we already indicate the obj nearby points in behav files so no need to add them again
                        end   

                        %% amplitude
                        mouselookobjectamplitudeexcel{iiiii+1,1+20}=iiiii;
                        mouselookobjectamplitudeexcel{iiiii+1,5*(i-1+kk)+3-1+20}=nameparts{i};
                        ...
                        ct3=ct3.*range1;
                        if isequal(objname{i001,1},nameset{1})
                            mouselookobjectamplitudeexcel{iiiii+1,5*(i-1+kk)+3+1-1+20}=sum(sum(ct3));%we already indicate the obj nearby points in behav files so no need to add them again
                        end
                        if isequal(objname{i001,1},nameset{2})
                            mouselookobjectamplitudeexcel{iiiii+1,5*(i-1+kk)+3+2-1+20}=sum(sum(ct3));%we already indicate the obj nearby points in behav files so no need to add them again
                        end
                        if isequal(objname{i001,1},nameset{3})
                            mouselookobjectamplitudeexcel{iiiii+1,5*(i-1+kk)+3+3-1+20}=sum(sum(ct3));%we already indicate the obj nearby points in behav files so no need to add them again
                        end
                        if isequal(objname{i001,1},nameset{4})
                            mouselookobjectamplitudeexcel{iiiii+1,5*(i-1+kk)+3+4-1+20}=sum(sum(ct3));%we already indicate the obj nearby points in behav files so no need to add them again
                        end  

                        %% amplitude norm
                        mouselookobjectamplituderateexcel{iiiii+1,1+20}=iiiii;
                        mouselookobjectamplituderateexcel{iiiii+1,5*(i-1+kk)+3-1+20}=nameparts{i};
                        ...
                        ct4=ct4.*range1;
                        if isequal(objname{i001,1},nameset{1})
                            mouselookobjectamplituderateexcel{iiiii+1,5*(i-1+kk)+3+1-1+20}=sum(sum(ct4));%we already indicate the obj nearby points in behav files so no need to add them again
                        end
                        if isequal(objname{i001,1},nameset{2})
                            mouselookobjectamplituderateexcel{iiiii+1,5*(i-1+kk)+3+2-1+20}=sum(sum(ct4));%we already indicate the obj nearby points in behav files so no need to add them again
                        end
                        if isequal(objname{i001,1},nameset{3})
                            mouselookobjectamplituderateexcel{iiiii+1,5*(i-1+kk)+3+3-1+20}=sum(sum(ct4));%we already indicate the obj nearby points in behav files so no need to add them again
                        end
                        if isequal(objname{i001,1},nameset{4})
                            mouselookobjectamplituderateexcel{iiiii+1,5*(i-1+kk)+3+4-1+20}=sum(sum(ct4));%we already indicate the obj nearby points in behav files so no need to add them again
                        end  

                        
                        %% count time
                        mouselookobjectcounttimeexcel{1,1+20}='not look obj';
                        mouselookobjectcounttimeexcel{iiiii+1,1+20}=iiiii;
                        mouselookobjectcounttimeexcel{iiiii+1,5*(i-1+kk)+3-1+20}=nameparts{i};
                        ct5=countTimemouob1;
                        ct5=ct5.*range1;

                        if isequal(objname{i001,1},nameset{1})
                             mouselookobjectcounttimeexcel{iiiii+1,5*(i-1+kk)+3+1-1+20}=sum(sum(ct5));
                        end
                        if isequal(objname{i001,1},nameset{2})
                             mouselookobjectcounttimeexcel{iiiii+1,5*(i-1+kk)+3+2-1+20}=sum(sum(ct5));
                        end
                        if isequal(objname{i001,1},nameset{3})
                             mouselookobjectcounttimeexcel{iiiii+1,5*(i-1+kk)+3+3-1+20}=sum(sum(ct5));
                        end
                        if isequal(objname{i001,1},nameset{4})
                             mouselookobjectcounttimeexcel{iiiii+1,5*(i-1+kk)+3+4-1+20}=sum(sum(ct5));
                        end                
                    end
                end
            end
