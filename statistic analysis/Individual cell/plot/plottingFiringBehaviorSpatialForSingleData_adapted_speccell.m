function plottingFiringBehaviorSpatialForSingleData_adapted_speccell(neuron,behavpos,behavtime,behavROI,firingRate,segment,threshFiring,threshSpatial,threshSpatial_2,threshSpatial_3,conditionfolder,sectionindex,binsize,tempname,behavcell,firingRate2,labfr2,firingRate3,labfr3,place_cells,TinfoPerSecond)
...
%     plot(neuron.S(i,:),'r')
    ...
    plot(neuron.pos(idx,1),neuron.pos(idx,2),'r.','MarkerSize',3)
    plot(0,0);
...
%     firingRateSmoothing = filter2DMatrices(ft1, 1);
    ...
    firingRateSmoothing(isnan(firingRateSmoothing))=0;
    firingRateSmoothing2 = nan(size(firingRateSmoothing)+1);
    firingRateSmoothing2(1:end-1,1:end-1) = firingRateSmoothing;
...
%         end
        if ~isempty(find(place_cells==i))
            title({'event count','(place cell)'},'FontName','Arial','FontSize',8,'FontWeight','bold')
        else
            title({'event count','(not place cell)'},'FontName','Arial','FontSize',8,'FontWeight','bold')
        end
...
% 
%     %      subplot(length(segment),3,3*k)
%     hand=subplot(numFig,plot_row*k-(plot_row-4));
% 
%     try
...
%     firingRateSmoothing = filter2DMatrices(ft3, 1);
    f = ones(2*radius-1)/sum(sum(ones(2*radius-1))); % this is average
    firingRateSmoothing=conv2(ft3,f,'same');
    firingRateSmoothing(isnan(firingRateSmoothing))=0;
    firingRateSmoothing2 = nan(size(firingRateSmoothing)+1);
    firingRateSmoothing2(1:end-1,1:end-1) = firingRateSmoothing;
    %      subplot(length(segment),3,3*k)
...
%         axis image
        % axis off
       if ~isempty(find(place_cells==i))
            title({labfr3,'(place cell)'},'FontName','Arial','FontSize',8,'FontWeight','bold')
        else
            title({labfr3,'(not place cell)'},'FontName','Arial','FontSize',8,'FontWeight','bold')
        end
        hold off
...
%         end
    ...
    if mod(kk,numFig) == 0 || kk == length(segment)
        saveas(gcf,fullfile(fpath,strcat(experimentName,'_CellFiringBeaviorSpatial_',num2str(i),'_binsize',num2str(binsize),tempname,'.fig')))
        saveas(gcf,fullfile(fpath,strcat(experimentName,'_CellFiringBeaviorSpatial_',num2str(i),'_binsize',num2str(binsize),tempname,'.tif')))
        saveas(gcf,fullfile(fpath,strcat(experimentName,'_CellFiringBeaviorSpatial_',num2str(i),'_binsize',num2str(binsize),tempname,'.eps')),'epsc')
        close all;
    end   
end
