function [firingRateAll,countAll,countTime] = calculatingCellSpatialForSingleData_adapted_amplitude(neuron,behavpos,behavtime,behavROI,binsize,Segment,threshold,temp,intensity,plotting,countTimeThresh)
%% Inputs:
...
countTime = countTime'/1000;%purpose? it makes the time in sec.
...
for k = 1:num
    ...
    if ~isempty(idx)
        count = countingFiringBins(idx,neuron,Segment(k),ampdata);
...
function count=countingFiringBins(idx,neuron,cellind,ampdata)
global pos1 pos2
...
for i = 1:length(idx)
    ...
    count(idxx,idyy) = count(idxx,idyy)+1*ampdata(cellind,idx(i));
    end
end
count = count';
