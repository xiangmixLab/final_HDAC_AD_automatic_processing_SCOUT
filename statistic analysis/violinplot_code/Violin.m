...
%% change to mean+-std box plot, 042819
            meann = mean(data(:));
%             se=std(data(:))/length(data)^0.5;
            stdd=std(data(:));
            ttiles =[meann-stdd,meann,meann+stdd];
            obj.BoxPlot = ... % plot color will be overwritten later
                fill([pos-0.01 pos+0.01 pos+0.01 pos-0.01], ...
                     [ttiles(1) ttiles(1) ttiles(3) ttiles(3)], ...
                     [1 1 1]);
            IQR = ttiles(3) - ttiles(1);
            lowhisker = ttiles(1);
            lowhisker = max(lowhisker, min(data));
            hiwhisker = ttiles(3);
            hiwhisker = min(hiwhisker, max(data));
            obj.WhiskerPlot = plot([pos pos], [lowhisker hiwhisker]);
            obj.MedianPlot = scatter(pos, ttiles(2), [], [1 1 1], 'filled');

            obj.NotchPlots = ...
                 scatter(pos, ttiles(2)-1.57*IQR/sqrt(length(data)), ...
                         [], [1 1 1], 'filled', '^');
            obj.NotchPlots(2) = ...
                 scatter(pos, ttiles(2)+1.57*IQR/sqrt(length(data)), ...
                         [], [1 1 1], 'filled', 'v');
