...
function alltracks = MotionBasedMultiObjectTrackingExample_120920(ai5)
...
%     displayTrackingResults([size(ai5,1) size(ai5,2)]);
...
function predictNewLocationsOfTracks()
    for i = 1:length(tracks)
        ...
        predictedArea = predict(tracks(i).kalmanFilter2);
        % Shift the bounding box so that its center is at 
        % the predicted location.
        predictedCentroid = int32(predictedCentroid) - bbox(3:4) / 2;
        tracks(i).bbox = [predictedCentroid, bbox(3:4)];            
        tracks(i).area = predictedArea;
...
function [assignments, unassignedTracks, unassignedDetections] = detectionToTrackAssignment()
    ...
    for i = 1:nTracks
%         cost(i, :) = distance(tracks(i).kalmanFilter1, centroids)+0.5*distance(tracks(i).kalmanFilter2, area);% larger cost means more unlikely be assigned to a current track
        cost(i, :) = distance(tracks(i).kalmanFilter1, centroids);% larger cost means more unlikely be assigned to a current track    
    end
...
function updateAssignedTracks()
    ...
    for i = 1:numAssignedTracks
        ...
        areat = area(detectionIdx, :);
        %Correct the estimate of the object's location
        %using the new detection.
        correct(tracks(trackIdx).kalmanFilter1, centroid);
        correct(tracks(trackIdx).kalmanFilter2, areat);

        %Replace predicted bounding box with detected
        %bounding box.
        tracks(trackIdx).bbox = bbox;
        tracks(trackIdx).area = areat;
...
% 
function displayTrackingResults(sz)
    ...
    mkdir(['Y:\Lujia\TBI project fig\Writing_folder\miniscope tiff','\','tracking illustration']);
    imwrite(frame,['Y:\Lujia\TBI project fig\Writing_folder\miniscope tiff','\','tracking illustration\b6250_frame',num2str(ct),'.tif']);
