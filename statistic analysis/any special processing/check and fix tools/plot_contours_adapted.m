function [CC,jsf,cm] = plot_contours_adapted(Aor,Cn,thr,display_numbers,max_number,Coor, ln_wd, ln_col)
...
if ~(nargin < 6 || isempty(Coor))
    CC = Coor;
    %%%%  added by Suoqin on 11-25-2016
    contAll = cell(size(Aor,2),1);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    for i = 1:size(Aor,2)
        cont = medfilt1(Coor{i}')';
        contAll{i} = cont;
        if size(cont,2) > 1
            plot(cont(1,2:end),cont(2,2:end),'Color','r', 'linewidth', ln_wd); hold on;
        end
    end
    %%%%  added by Suoqin on 11-25-2016
%     save contoursAll.mat contAll
    %%%%%%%%%%%%%%
else
...
if display_numbers
    ...
    text(round(cm(1:max_number,2)),round(cm(1:max_number,1)),lbl(1:max_number),'color',[0,0,0],'fontsize',12,'fontname',fontname,'fontweight','bold');
   % plot(cm(1:max_number,2),cm(1:max_number,1),'ro'); % centroid of conrours. 
end
%%%%%%%%%% added by Suoqin on 11-25-2016
% lbl = strtrim(cellstr(num2str((1:size(Aor,2))')));
% save centroid.mat cm;
% save labelNeuron.mat lbl;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
axis off;
if ~(nargin < 6 || isempty(Coor))
    jsf = [];
