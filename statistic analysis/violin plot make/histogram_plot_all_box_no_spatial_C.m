...
%     group={[2 3 9 14 22 23 19],[1 4 11 12 17 18 20]};
    ...
    savedir='C:\Users\exx\Desktop\HDAC paper fig and method\fig2\histogram_single_cell_allbox_no_spatial';
...
%     ylabel('cell num');
    ...
    a04{ct,g}=sum(dat_cs_sig,2);%amp
   
    a05{ct,g}=sum(dat_cs_sig,2)/size(dat_cs,2);%normalized_amp
    ct=ct+1;
end
...
for i=1:3
 ...
 struct_amp_12.Control_young=control_young_amp{i,1};
 struct_amp_12.Control_old=control_old_amp{i,1};
 struct_amp_12.RGFP_old=rgfp_old_amp{i,1};
...
%     rk=ranksum(a01,a02);
    ...
    close;
     ...
     k01=struct_amp_12.Control_young;
    k02=struct_amp_12.Control_old;
    k03=struct_amp_12.RGFP_old;
    max_size=max([length(k01),length(k02),length(k03)]);
    if length(k01)<max_size
        k01(length(k01)+1:max_size)=nan;
    end
...
%     rk=ranksum(a01,a02);
   ...
   set(gcf,'renderer','painters');
    saveas(gcf,[savedir,'\',conditionname{i},'_amp_no_spatial_',nam,'violin.eps'],'epsc');
    saveas(gcf,[savedir,'\',conditionname{i},'_amp_no_spatial_',nam,'violin.tif'],'tif');
    saveas(gcf,[savedir,'\',conditionname{i},'_amp_no_spatial_',nam,'violin.fig'],'fig');    
    close
 end
