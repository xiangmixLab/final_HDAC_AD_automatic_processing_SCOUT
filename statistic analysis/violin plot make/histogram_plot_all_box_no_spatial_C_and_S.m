...
%     group={[2 3 9 14 22 23 19],[1 4 11 12 17 18 20]};
    ...
    savedir='C:\Users\exx\Desktop\HDAC paper fig and method\fig2\histogram_single_cell_allbox_no_spatial_c_s';
...
for j=group{g}
    ...
    dat_cs=neuronIndividuals_new{i}.S;
    nam='S';        
...
%     ylabel('cell num');
 ...
 exname=[savedir,'\',groupname{g},' individual cell distribution all box no spatial ',nam,'.xlsx'];
 sname1=[conditionname{i},'_count_events'];
 sname2=[conditionname{i},'_firing_rate'];
 sname3=[conditionname{i},'_amplitude'];
 sname4=[conditionname{i},'_amplitude_norm'];
 sheetcol={'A','B','C','D','E'};
...
%     rk=ranksum(a01,a02);
   ...
   set(gcf,'renderer','painters');
    saveas(gcf,[savedir,'\',conditionname{i},'_countEvents_no_spatial_',nam,'violin.eps'],'epsc');
    saveas(gcf,[savedir,'\',conditionname{i},'_countEvents_no_spatial_',nam,'violin.tif'],'tif');
    saveas(gcf,[savedir,'\',conditionname{i},'_countEvents_no_spatial_',nam,'violin.fig'],'fig');
    close;
...
%     rk=ranksum(a01,a02);
    ...
    set(gcf,'renderer','painters');
    saveas(gcf,[savedir,'\',conditionname{i},'_firingRate_no_spatial_',nam,'violin.eps'],'epsc');
    saveas(gcf,[savedir,'\',conditionname{i},'_firingRate_no_spatial_',nam,'violin.tif'],'tif');
    saveas(gcf,[savedir,'\',conditionname{i},'_firingRate_no_spatial_',nam,'violin.fig'],'fig');
    close;

 %% violin
 if exp==10

    struct_event_10.Control_old=control_old_cevents{i,1};
    struct_event_10.RGFP_old=rgfp_old_cevents{i,1};

    struct_rate_10.Control_old=control_old_rate{i,1};
    struct_rate_10.RGFP_old=rgfp_old_rate{i,1};
    

    colorGroup = [200 0 0;0 0 200;0 212 212]/255;

    k02=struct_event_10.Control_old;
    k03=struct_event_10.RGFP_old;
    max_size=max([length(k02),length(k03)]);
    if length(k02)<max_size
        k02(length(k02)+1:max_size)=nan;
    end
    if length(k03)<max_size
    ...
    t = table([1:length(k02)]',k02,k03,'VariableNames',{'cells','meas1','meas2'});
    Meas = table([1 2]','VariableNames',{'Measurements'});
    rm = fitrm(t,'meas1-meas2~cells','WithinDesign',Meas);
    ranovatbl = ranova(rm);
    rk=ranovatbl{1,7};% Huynh-Feldt correction
    close; 
    figure;
 h = violinplot(struct_event_10);
...
%     rk=ranksum(a01,a02);
    if rk<0.05
        text(1.2,max([max(k02),max(k03)])*2/3,['sig. difference across 3 groups Huynh-Feldt corrected p=',num2str(rk)]);
    else
        text(1.2,max([max(k02),max(k03)])*2/3,['No sig. difference across 3 groups Huynh-Feldt corrected p=',num2str(rk)]);
    end
   set(gcf,'renderer','painters');
    saveas(gcf,[savedir,'\',conditionname{i},'_injection_countEvents_no_spatial_',nam,'violin.eps'],'epsc');
    saveas(gcf,[savedir,'\',conditionname{i},'_injection_countEvents_no_spatial_',nam,'violin.tif'],'tif');
    saveas(gcf,[savedir,'\',conditionname{i},'_injection_countEvents_no_spatial_',nam,'violin.fig'],'fig');
    close;
    
    k02=struct_rate_10.Control_old;
    k03=struct_rate_10.RGFP_old;
    max_size=max([length(k02),length(k03)]);
    if length(k02)<max_size
        k02(length(k02)+1:max_size)=nan;
    end
...
%     rk=ranksum(a01,a02);
   ...
   set(gcf,'renderer','painters');
    saveas(gcf,[savedir,'\',conditionname{i},'_injection_firingrates_no_spatial_',nam,'violin.eps'],'epsc');
    saveas(gcf,[savedir,'\',conditionname{i},'_injection_firingrates_no_spatial_',nam,'violin.tif'],'tif');
    saveas(gcf,[savedir,'\',conditionname{i},'_injection_firingrates_no_spatial_',nam,'violin.fig'],'fig');
    close;
end
end
end
