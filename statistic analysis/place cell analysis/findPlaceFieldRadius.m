function [PF_radius,PF_radius_sum]= findPlaceFieldRadius(autocorr, auto_max_inds)
...
PF_radius_sum_list=[]
for p=1:length(new_distances)
    PF_radius_sum_list(p)=(new_distances(p)/2)*0.7;
end

PF_radius_sum=nansum(PF_radius_sum_list);