function [place_cells,infoScoreThresh,infoScoreSecondboot, infoScoreSpikeboot,infonull] = permutingSpike_Suoqin_2019(file_neuronIndividuals,sectionIndex,neuron,behav,thresh,temp,deltaTall,occThresh,nboot,binsize,session,neuron_lowFR)
...
[firingrateAll,countAll,countTime] = calculatingCellSpatialForSingleData(neuron,behav,1:size(neuron.C,1),thresh,binsize,temp,[],[]);
...
neuron = neuronIndividuals{sectionIndex}.copy;
...
for nE = 1:nboot
   ...
   % [firingrateAll,countAll,countTime] = calculatingCellSpatialLinearTrackForSingleData2(neuronboot,behav,1:size(neuronboot.trace,1),thresh,temp);
    [firingrateAll,countAll,countTime] = calculatingCellSpatialForSingleData(neuronboot,behav,1:size(neuron.C,1),thresh,binsize,temp,[],[]);
...
% TinfoPerSecond = table([1:length(infoPerSecondnull)]',infoPerSecondnull,infoScoreThresh,'VariableNames',{'neuron','infoPerSecond','thresh'});
