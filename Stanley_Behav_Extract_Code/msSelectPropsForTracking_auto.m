...
function [behav,frame,frame1_bg] = msSelectPropsForTracking_auto(behav,ROImethod,ListedROI,o1,cspace)
...
%   Detailed explanation goes here
    ...
    for i=1:10:behav.numFrames
        ...
        % check color space and use different processing
        switch cspace
            case 'lab'
                if size(rect,1)==1&&size(rect,2)==4
            ...
            case 'rgb'
                if size(rect,1)==1&&size(rect,2)==4
                    t_lab=(t(round(rect(2)):round(rect(2)+rect(4)),round(rect(1)):round(rect(1)+rect(3)),:));
                else
                    t_lab=(t.*double(rect));
                end
                t_lab_r=t_lab(:,:,1)-t_lab(:,:,2);
                t_lab_b=t_lab(:,:,3)-t_lab(:,:,2);
                max_framer(count)=max(max(t_lab_r));
                max_frameb(count)=max(max(t_lab_b));
            otherwise
        end
...
%     rgbLevelsb=nan(behav.numFrames,6);
    parfor i=2:behav.numFrames
        [hsvLevelsr(i,:),rgbLevelsr(i,:),hsvLevelsb(i,:),rgbLevelsb(i,:),rcentroid(i,:),bcentroid(i,:)]=LED_position_extract(behav,i,frame1_bg,rect,se,se1,max_framer1,max_frameb1,cspace);       
        toc;        
    end
