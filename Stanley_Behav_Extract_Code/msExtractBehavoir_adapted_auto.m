function behav = msExtractBehavoir_adapted_auto(behav, trackLength)
...
%%
    ...
    position(1,:)=[nan nan];% first frame might be the recording from last run
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% blue LED
    positionblue(1,:)=[nan nan];% first 5 frame might be the recording from last run
...
%    [positionblue(:,1) positionblue(:,2)]=smooth_contours(positionblue(:,1), positionblue(:,2), ceil(1/dt));
    positionblue = smoothdata(positionblue,'SmoothingFactor',0.01);   
