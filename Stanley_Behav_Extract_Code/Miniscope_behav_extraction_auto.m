...
%objInput: Y,N,List
function [behavfile_list,camera_id_list]=Miniscope_behav_extraction_auto(orilocation,vname,behavVidprefix,mrange,ROImethod,ROIlist,objInput,objlist,trackLengtht,cspace)
tic;
behavfile_list={};
camera_id_list=[];
for ol=mrange
...
[behav,framest,bg] = msSelectPropsForTracking_auto(behav,ROImethod,ROIlist,ol,cspace); 
size(framest)
%% TRACK LENGTH
...
if ~iscell(trackLengtht)
    if size(framest,2)>size(framest,1)
        trackLength=max(trackLengtht);
    else
        trackLength=min(trackLengtht);
    end
else
    if size(framest,2)>size(framest,1)
        trackLength=max(trackLengtht{ol});
    else
        trackLength=min(trackLengtht{ol});
    end
end
%% Select Objects
...
if (strcmp(objInput,'List'))&&~isempty(objlist)% object: just maintain the coordination selected from imagej or ginput
    if sum(objlist{ol})>0
        for lkk=1:size(objlist{ol},1)
            co(lkk,:)=[objlist{ol}(lkk,1)-behav.ROI(1),behav.ROI(2)+behav.ROI(4)-objlist{ol}(lkk,2)]; % to [x,y] coordination (left down 0)
        end
    else
        co=[0,0];
    end
...
behav = msExtractBehavoir_adapted_auto(behav, trackLength); 
% 
...
filename1=[vname{ol},'_',behavVidprefix];
% filename1=[vname{ol}];

%% Check behav trace using the lines below. Include figures into your notes
figure; hist(diff(behav.time))
saveas(gcf,[behavVidprefix,'behavtimeinterval.tif']);
saveas(gcf,[behavVidprefix,'behavtimeinterval.fig']);
figure; 
...
axis image
saveas(gcf,[behavVidprefix,'behavtrack_auto_red.png']);
saveas(gcf,[behavVidprefix,'behavtrack_auto_red.fig']);
figure; 
imshow(framest);hold on;
rectangle('Position',behav.ROI,'LineWidth',2);
plot(behav.positionblue(:,1)*behav.ROI(3)/trackLength+behav.ROI(1),behav.positionblue(:,2)*behav.ROI(3)/trackLength+behav.ROI(2))
if sum(behav.object)>0
    plot(behav.object(:,1)+behav.ROI(1),behav.ROI(4)+behav.ROI(2)-behav.object(:,2),'.','MarkerSize',20) % THIS MAKES OBJ [I J] COORDINATION WHEN PLOT, SO OBJ IS [X,Y] COORDINATION
end
axis image
saveas(gcf,[behavVidprefix,'behavtrack_auto_blue.tif']);
saveas(gcf,[behavVidprefix,'behavtrack_auto_blue.fig']);
close all;
