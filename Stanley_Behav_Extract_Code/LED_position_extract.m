function [hsvLevelsr,rgbLevelsr,hsvLevelsb,rgbLevelsb,rcentroid,bcentroid]=LED_position_extract(behav,i,frame1_bg,rect,se,se1,max_framer,max_frameb,cspace)       
        ...
        if length(size(tempf))>3
            tempf=tempf-squeeze(frame1_bg(:,:,:,min(ceil(i/40),floor(behav.numFrames/40))))-tempf(:,:,length(size(tempf))-1);
        else
            if length(size(tempf))==3
                tempf=tempf-frame1_bg-tempf(:,:,2);
            else
        ...
        switch cspace
            case 'rgb'
                frame2_r=squeeze(frame2(:,:,1))-squeeze(frame2(:,:,2));
                frame2_b=squeeze(frame2(:,:,3))-squeeze(frame2(:,:,2));
            case 'lab'
...
%         tempf2_s=imgaussfilt(tempf2,3,'FilterSize',[15 15]);
        ...
        if max_framer>0
            ...
            bwb=tempf2_s>max(tempf2_s(:))*thres_blue;
        end
        bwb=imdilate(bwb,se);
