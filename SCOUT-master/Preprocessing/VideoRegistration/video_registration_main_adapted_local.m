function [num2read,fname,data_shape]=video_registration_main_adapted_local(use_non_rigid,base_index,projection_type,automatic,projection_dir,data_type,vid_path_in)
...
registration_methods={'translation'};

if use_non_rigid
    registration_methods{end+1}='non-rigid';
end

vid_paths=dir('1204*metric.mat');
...
if length(projections)==0
    if isequal(projection_type,'max')
        ...
        projections=construct_correlation_template_main_adapted(data_type,vid_paths);
    end
end
...
Y=uint8(cat(3,aligned{:}));
Ysiz=size(Y);
[path]=fileparts(vid_paths{1});
fname=[path,'\','final_concatenate_stack.mat'];
num2read=zeros(1,length(aligned));
for i=1:length(aligned)
    num2read(i+1)=size(vids{i},3);
end
num2read(1)=sum(num2read(2:end));
data_shape=size(aligned{1});
data_shape=data_shape(1:2);

save(fullfile(path,'final_concatenate_stack.mat'),'Y','Ysiz','-v7.3');
save(fullfile(path,'final_concatenate_stack_stats.mat'),'num2read','data_shape');
