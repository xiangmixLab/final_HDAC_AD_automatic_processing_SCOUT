...
%         if sum(isnan(temp))>1500 % 100sec is nan, added 11/01/2020
%             ind_del(k)=true;
%             continue;
%         end
        ...
        %% added 052019, steve's data may contain bad values
        if sum(ck(2:end))==inf || isnan(sum(ck(2:end)))
            ck=zeros(1,size(ck,1))';
        end
