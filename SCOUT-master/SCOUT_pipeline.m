function []=SCOUT_pipeline(vid_files,base_dir,batch_sizes,extraction_options,cell_tracking_options)

%Global cell tracking parameters
cd(base_dir);
overlap_per_batch=1500; %(Set to 0 for standard cell tracking (with vids_per_batch=1) ) % 100sec overlap
threads=8; %Number of sessions to extract simultaneously, high values can easily exceed memory threshold, but significantly boost speed
data_type='1p';
