...
%JS: Jensen-Shannon divergence threshold

function neuron_out=demo_spatialfilter_func(fname)
...
gSiz=gSiz/2;
gSizMin=4;          % minimum diameter of neurons in the image plane.
...
for i=1:size(neuron.A,2)
    Ai=nA(:,i);
    Ai=Ai*1/max(Ai(:));
    ind=find(Ai>0.5);%the pixels at which a neuron shows up
    Atemp=Ai*0;
    Atemp(ind)=Ai(ind);
    nA(:,i)=Atemp;
end
[neuron.Coor,json_file,centroid] = plot_contours(nA, Cn, 0.8, 1, [], [], 2);
