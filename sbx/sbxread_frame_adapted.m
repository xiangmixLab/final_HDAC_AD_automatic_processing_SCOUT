function [x,N] = sbxread_frame_adapted(fname,k,N,varargin)
...
if(isfield(info,'fid') && info.fid ~= -1)
    ...
    try
        ...
        if ~isempty(N)
            ...
            x = fread(info.fid,info.nsamples/2 * (filelength/(info.nsamples)-1),'uint16=>uint16');
            N=(filelength/(info.nsamples)-1);
        end
